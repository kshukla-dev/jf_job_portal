      ...
      <div className={styles.hero}>
        ...
        <Container>
          <div className={styles.heroInner}>
            <span className={styles.badge}>{t('hero_eyebrow_live', 'Live Career Openings')}</span>
            <h1 className={styles.title}>{t('jobs_title', 'Global Job Opportunities')}</h1>
            <p className={styles.subtitle}>
              {t('jobs_sub', 'Explore thousands of curated roles across 50+ countries. Find international positions in software engineering, banking, healthcare, and executive management.')}
            </p>
          </div>
        </Container>
      </div>
          ...
          <div className={styles.headerBar}>
            <h2 className={styles.sectionTitle}>
              {searchTerm || selectedLocation || selectedType || activeChip
                ? `${t('showing_vacancies', 'Search Results')}: ${filteredJobs.length}`
                : `${t('jobs_title', 'Featured Worldwide Openings')} (${filteredJobs.length})`}
            </h2>
